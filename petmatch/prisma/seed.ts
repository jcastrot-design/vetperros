...
async function main() {
  ...
  const escuelaCanina = await prisma.user.create({
    data: {
      email: "info@escuelacanina.cl",
      name: "Escuela Canina Profesional",
      passwordHash,
      role: "WALKER",
      city: "Providencia",
    },
  ...
  const dogMaster = await prisma.user.create({
    data: {
      email: "hola@dogmaster.cl",
      name: "Dog Master Chile",
      passwordHash,
      role: "WALKER",
      city: "Las Condes",
    },
  ...
  const escuelaProfile = await prisma.providerProfile.create({
    data: {
      ...
      experience: 7,
      certifications: JSON.stringify(["Adiestramiento con refuerzo positivo", "Curso para dueños primerizo"]),
      photos: JSON.stringify([]),
      coverageRadius: 15,
      verificationStatus: "VERIFIED",
      verifiedAt: new Date(),
  ...
  await prisma.service.createMany({
    data: [
      ...
      },
      {
        ...
        description: "Aprende a entender el lenguaje canino y comunicarte efectivamente con tu mascota. Técnicas de refuerzo positivo, manejo del correa y resolución de problemas comunes.",
        pricePerUnit: 18000,
        city: "Providencia",
        latitude: -33.4254,
        longitude: -70.6156,
